<?php
    foreach($useful_link_data as $row){
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/useful_link/update/' . $row['useful_link_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group btm_border">
                        <label class="col-sm-4 control-label" for="useful_link_title"><?php echo translate('useful_link_title');?></label>
                        <div class="col-sm-6">
                            <input type="text" name="title" id="title" 
                                placeholder="<?php echo translate('useful_link_title');?>" class="form-control required" value="<?php echo $row['title']; ?>">
                        </div>
                    </div>


                <div class="form-group abstract">
                        <label class="col-sm-4 control-label"><?php echo translate('user_link_summary');?></label>
                        <div class="col-sm-6">
                            <textarea class="summernotes"  data-height="200" data-name="summary" ><?php echo $row['summary'];?></textarea>
                        </div>
                   </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="useful_link_link"><?php echo translate('useful_link_link');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="link" id="link" 
                            placeholder="<?php echo translate('useful_link_link');?>" class="form-control required" value="<?php echo $row['link']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_useful_link'); ?>','<?php echo translate('successfully_edited!'); ?>','useful_link_edit','<?php echo $row['useful_link_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('useful_link_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.useful_link_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#useful_link_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#useful_link_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="useful_link"><?php echo translate('useful_link'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 useful_link" style="display:none;">'
				+'				<select class="demo-cs-multiselect useful_link_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_useful_link[]" class="hidd" />'
        ...
        $('body').on('change', '.useful_link_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'useful_link'){
                h.parent().parent().find('.useful_link').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.useful_link').hide();
