...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/gallery/update/' . $row['gallery_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group btm_border">
                        <label class="col-sm-4 control-label" for="gallery_title"><?php echo translate('gallery_title');?></label>
                        <div class="col-sm-6">
                            <input type="text" name="title" id="title" 
                                placeholder="<?php echo translate('gallery_title');?>" class="form-control required" value="<?php echo $row['title']; ?>">
                        </div>
                    </div>
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="gallery_status"><?php echo translate('gallery_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('gallery_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_gallery'); ?>','<?php echo translate('successfully_edited!'); ?>','gallery_edit','<?php echo $row['gallery_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('gallery_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.gallery_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="gallery"><?php echo translate('gallery'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 gallery" style="display:none;">'
				+'				<select class="demo-cs-multiselect gallery_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_gallery[]" class="hidd" />'
        ...
        $('body').on('change', '.gallery_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'gallery'){
                h.parent().parent().find('.gallery').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.gallery').hide();
