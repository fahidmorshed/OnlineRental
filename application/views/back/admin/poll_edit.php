...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/poll/update/' . $row['poll_id'], array(
            ...
            ?>
                <div class="panel-body">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="poll_option"><?php echo translate('poll_options');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="options" id="options" 
                            placeholder="<?php echo translate('poll_options');?>" class="form-control required" value="<?php echo $row['options']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="poll_status"><?php echo translate('poll_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('poll_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.poll_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="poll"><?php echo translate('poll'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 poll" style="display:none;">'
				+'				<select class="demo-cs-multiselect poll_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_poll[]" class="hidd" />'
        ...
        $('body').on('change', '.poll_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'poll'){
                h.parent().parent().find('.poll').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.poll').hide();
