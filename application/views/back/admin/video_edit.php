...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/video/update/' . $row['video_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group btm_border">
                        <label class="col-sm-4 control-label" for="video_title"><?php echo translate('video_title');?></label>
                        <div class="col-sm-6">
                            <input type="text" name="title" id="title" 
                                placeholder="<?php echo translate('video_title');?>" class="form-control required" value="<?php echo $row['title']; ?>">
                        </div>
                    </div>


                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="videotype"><?php echo translate('video_type');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="type" id="type" 
                            placeholder="<?php echo translate('video_type');?>" class="form-control required" value="<?php echo $row['type']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="video_link"><?php echo translate('video_link');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="link" id="link" 
                            placeholder="<?php echo translate('video_link');?>" class="form-control required" value="<?php echo $row['link']; ?>">
                    </div>
                </div>

                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="video_status"><?php echo translate('video_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('video_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_video'); ?>','<?php echo translate('successfully_edited!'); ?>','video_edit','<?php echo $row['video_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('video_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.video_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="video"><?php echo translate('video'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 video" style="display:none;">'
				+'				<select class="demo-cs-multiselect video_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_video[]" class="hidd" />'
        ...
        $('body').on('change', '.video_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'video'){
                h.parent().parent().find('.video').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.video').hide();
